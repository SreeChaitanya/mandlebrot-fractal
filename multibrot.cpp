...
int mandelbrot(float C_re, float C_im, int max_count)
{
  ...
  {
    /* float N_re = pow(Z_re * Z_re + Z_im * Z_im,m/2)*cos(m*atan2(Z_im,Z_re)) + C_re;
    float N_im = pow(Z_re * Z_re + Z_im * Z_im,m/2)*sin(m*atan2(Z_im,Z_re))+ C_im;
    Z_re = N_re;
    Z_im = N_im;
